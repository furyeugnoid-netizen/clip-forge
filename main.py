...
PAYMENT_RATE = 0.001  # Default - changeable via command
...
class ConnectSocialsView(discord.ui.View):
    ...
    async def link_account(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(
            "Use: `!linkaccount [platform] [@username]`\n\n"
            "Example: `!linkaccount youtube @MyChannel`\n\n"
            "**Supported platforms:** youtube, instagram, tiktok, x",
            ephemeral=True
        )
    ...
    async def view_accounts(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if user_id in db["users"] and db["users"][user_id].get("accounts"):
            ...
            embed = discord.Embed(
                ...
                description=text or "No accounts linked yet",
                color=LIGHT_CYAN
            )
    ...
    async def verify_demo(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(
            "Demographics verification coming soon! ✨\n"
            "This will help us track your audience insights.",
            ephemeral=True
        )
...
class YourAccountView(discord.ui.View):
    ...
    async def social_accounts(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if user_id in db["users"] and db["users"][user_id].get("accounts"):
            ...
            embed = discord.Embed(
                title="Your Linked Accounts",
                description=text or "No accounts linked",
                color=CYAN
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
        else:
...
async def account(ctx):
    """View your account & earnings"""
    embed = discord.Embed(
        title="👤 Your Account",
        description="Manage your clips and earnings",
        color=LIGHT_CYAN
    )
    embed.set_footer(text="Powered by ❤️ & ☕ | Clip Forge")
...
async def submitclip(ctx, platform: str, url: str, views: int):
    """Submit a clip for monetization"""
    user_id = str(ctx.author.id)
    ...
    clip = {
        ...
        "earnings": views * db["payment_rate"]
    }
...
async def leaderboard(ctx):
    ...
    embed = discord.Embed(
        ...
        description=text or "No earnings yet",
        color=DARK_BLUE
    )
    embed.set_footer(text="Powered by ❤️ & ☕ | Clip Forge")
    await ctx.send(embed=embed)
...
async def setpaymentrate(ctx, rate: float):
    """Set payment rate (Admin only)"""
    db["payment_rate"] = rate
    save_database(db)
    
    embed = discord.Embed(
        title="✅ Payment Rate Updated",
        description=f"New rate: ${rate} per view",
        color=CYAN
    )
    await ctx.send(embed=embed)
...
async def help_command(ctx):
    ...
    embed.add_field(name="🔧 Admin Commands", value=
        "`!setpaymentrate [amount]` - Set payment rate\n"
